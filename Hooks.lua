...
function LUIE.InitializeHooks()
    ...
    GetSkillAbilityInfo = function(skillType, skillIndex, abilityIndex)
        local name, texture, earnedRank, passive, ultimate, purchased, progressionIndex, rankIndex =
            zos_GetSkillAbilityInfo(skillType, skillIndex, abilityIndex)
    ...
    GetUnitBuffInfo = function(unitTag, buffIndex)
        local buffName, startTime, endTime, buffSlot, stackCount, iconFile, buffType, effectType, abilityType, statusEffectType, abilityId, canClickOff, castByPlayer =
            zos_GetUnitBuffInfo(unitTag, buffIndex)
        if LUIE.Data.Effects.EffectOverride[abilityId] and LUIE.Data.Effects.EffectOverride[abilityId].name then
    ...
    DoesKillingAttackHaveAttacker = function(index)
        ...
        local attackName, attackDamage, attackIcon, wasKillingBlow, castTimeAgoMS, durationMS, numAttackHits, abilityId =
            zos_GetKillingAttackInfo(index)
    ...
    GetKillingAttackerInfo = function(index)
        local attackerRawName, attackerChampionPoints, attackerLevel, attackerAvARank, isPlayer, isBoss, alliance, minionName, attackerDisplayName =
            zos_GetKillingAttackerInfo(index)
        local attackName, attackDamage, attackIcon, wasKillingBlow, castTimeAgoMS, durationMS, numAttackHits, abilityId =
            zos_GetKillingAttackInfo(index)
    ...
    GetArtificialEffectInfo = function(artificialEffectId)
        local displayName, iconFile, effectType, sortOrder, timeStarted, timeEnding =
            zos_GetArtificialEffectInfo(artificialEffectId)
        if
            LUIE.Data.Effects.ArtificialEffectOverride[artificialEffectId]
            and LUIE.Data.Effects.ArtificialEffectOverride[artificialEffectId].icon
        then
            iconFile = LUIE.Data.Effects.ArtificialEffectOverride[artificialEffectId].icon
        end
        if
            LUIE.Data.Effects.ArtificialEffectOverride[artificialEffectId]
            and LUIE.Data.Effects.ArtificialEffectOverride[artificialEffectId].name
        then
    ...
    GetArtificialEffectTooltipText = function(artificialEffectId)
        ...
        if
            LUIE.Data.Effects.ArtificialEffectOverride[artificialEffectId]
            and LUIE.Data.Effects.ArtificialEffectOverride[artificialEffectId].tooltip
        then
    ...
    -- Hook Action Slots
    local ACTION_BUTTON_BGS =
        { ability = "EsoUI/Art/ActionBar/abilityInset.dds", item = "EsoUI/Art/ActionBar/quickslotBG.dds" }
    ...
    local function SetupActionSlot(slotObject, slotId)
        ...
        ZO_ActionSlot_SetupSlot(
            slotObject.icon,
            slotObject.button,
            slotIcon,
            isGamepad and "" or ACTION_BUTTON_BORDERS.normal,
            isGamepad and "" or ACTION_BUTTON_BORDERS.mouseDown,
            slotObject.cooldownIcon
        )
    ...
    ActionButton.UpdateActivationHighlight = function(self)
        ...
        local showHighlight = not slotIsEmpty
            and (HasActivationHighlight(slotnum, hotbarCategory) or LUIE.Data.Effects.IsAbilityActiveGlow[abilityId] == true)
            and not self.useFailure
            and not self.showingCooldown
    ...
    ActionButton.UpdateState = function(self)
        ...
        local hidden = true
        if
            IsSlotToggled(slotnum, hotbarCategory) == true
            or LUIE.Data.Effects.IsAbilityActiveHighlight[abilityId] == true
        then
            hidden = false
        end
    ...
    local function GetDefensiveBonusString(campaignId)
        local _, enemyScrollsHeld =
            GetAvAArtifactScore(campaignId, GetUnitAlliance("player"), OBJECTIVE_ARTIFACT_DEFENSIVE)
    ...
    local function GetDefensiveBonusScore(campaignId)
        local allHomeScrollsHeld, enemyScrollsHeld =
            GetAvAArtifactScore(campaignId, GetUnitAlliance("player"), OBJECTIVE_ARTIFACT_DEFENSIVE)
    ...
    local function GetOffensiveBonusString(campaignId)
        local _, enemyScrollsHeld =
            GetAvAArtifactScore(campaignId, GetUnitAlliance("player"), OBJECTIVE_ARTIFACT_OFFENSIVE)
        return zo_strformat(SI_CAMPAIGN_BONUSES_ENEMY_SCROLL_INFO, enemyScrollsHeld)
    end
    ...
    ZO_CampaignBonuses_Shared.BuildMasterList = function(self)
        ...
        for bonusType, info in ipairs(BONUS_SECTION_DATA) do
            ...
            for i = startIndex, count do
                ...
                local icon = (
                    LUIE.Data.Effects.EffectOverride[abilityId]
                    and LUIE.Data.Effects.EffectOverride[abilityId].passiveIcon
                )
                        and LUIE.Data.Effects.EffectOverride[abilityId].passiveIcon
                    or GetAbilityIcon(abilityId) -- Get Updated LUIE AbilityIcon here
        ...
        if self.abilityList:IsActive() then
            ...
            if targetData and targetData.isHeader == false then
                ...
                if targetData.abilityId then
                    ...
                    if
                        LUIE.Data.Effects.EffectOverride[abilityId]
                        and LUIE.Data.Effects.EffectOverride[abilityId].tooltip
                    then
        ...
        for currentLevel = 0, GetKeepMaxUpgradeLevel(self.keepUpgradeObject:GetKeep()) do
            ...
            if numUpgrades > 0 then
                ...
                for i = 1, numUpgrades do
                    local name, description, icon, atPercent, isActive =
                        self.keepUpgradeObject:GetLevelUpgradeInfo(currentLevel, i)
    ...
    function ZO_Skills_AbilityEntry_Setup(control, skillData)
        ...
        if skillPointAllocationMode == SKILL_POINT_ALLOCATION_MODE_PURCHASE_ONLY then
            ...
            if skillData:CanPointAllocationsBeAltered(isFullRespec) then
                ...
                if
                    skillPointAllocator:CanSell()
                    or skillPointAllocator:CanDecreaseRank()
                    or skillPointAllocator:CanUnmorph()
                then
    ...
    local function InitializeKeyboardConfirmDialog()
        ...
        local function SetupPurchaseAbilityConfirmDialog(dialog, skillProgressionData)
            if skillProgressionData:GetSkillData():GetPointAllocator():CanPurchase() then
                ...
                local hideAdvisement = ZO_SKILLS_ADVISOR_SINGLETON:IsAdvancedModeSelected()
                    or not skillProgressionData:IsAdvised()
    ...
    local function SetupAbilityIconFrame(control, isPassive, isActive, isAdvised)
        ...
        if isActive then
            ...
            if isAdvised then
                frameOffsetFromIcon = DOUBLE_FRAME_THICKNESS
                edgeFrameBackdrop:SetEdgeTexture(
                    "EsoUI/Art/SkillsAdvisor/gamepad/edgeDoubleframeGamepadBorder.dds",
                    SKILLS_ADVISOR_ACTIVE_DOUBLE_FRAME_WIDTH,
                    SKILLS_ADVISOR_ACTIVE_DOUBLE_FRAME_HEIGHT
                )
            else
                frameOffsetFromIcon = SINGLE_FRAME_THICKNESS
                edgeFrameBackdrop:SetEdgeTexture(
                    "EsoUI/Art/Miscellaneous/Gamepad/edgeframeGamepadBorder.dds",
                    SKILLS_ADVISOR_ACTIVE_DOUBLE_FRAME_WIDTH,
                    SKILLS_ADVISOR_ACTIVE_DOUBLE_FRAME_HEIGHT
                )
            end
    ...
    local function SetBindingTextForSkill(keybindLabel, skillData)
        ...
        if
            SKILLS_AND_ACTION_BAR_MANAGER:GetSkillPointAllocationMode() == SKILL_POINT_ALLOCATION_MODE_PURCHASE_ONLY
            and skillData:IsActive()
        then
            ...
            if slot then
                ...
                local actionName =
                    ACTION_BAR_ASSIGNMENT_MANAGER:GetActionNameForSlot(slot, hotbarCategory, GAMEPAD_MODE)
                local bindingText = ""
                if actionName then
                    bindingText = ZO_Keybindings_GetHighestPriorityBindingStringFromAction(
                        actionName,
                    ...
                    bindingText =
                        zo_iconFormat("EsoUI/Art/Inventory/Gamepad/gp_inventory_icon_equipped.dds", "100%", "100%")
                    keybindWidth = 50 --width minus single keybind
                end
    ...
    local function SetupIndicatorsForSkill(
        leftIndicator,
        isMorph,
        rightIndicator,
        skillData,
    ...
    function ZO_GamepadSkillEntryTemplate_Setup(control, skillEntry, selected, activated, displayView)
        ...
        local indicatorWidth = SetupIndicatorsForSkill(
            control.leftIndicator,
            control.rightIndicator,
            skillData,
            showIncrease,
            showDecrease,
            showNew
