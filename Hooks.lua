...
function LUIE.InitializeHooks()
    ...
    GetKillingAttackInfo = function (index)
        ...
        if attackName == GetString(LUIE_SKILL_FALL_DAMAGE) then
    ...
    RequestFriend = function (option1, option2, menu)
        ...
        if not menu then
            local message = zo_strformat(GetString(LUIE_SLASHCMDS_FRIEND_INVITE_MSG), option1)
    ...
    AddIgnore = function (option)
        ...
        if IsIgnored(option) then -- Only lists account names, unfortunately
            printToChat(GetString(LUIE_SLASHCMDS_IGNORE_FAILED_ALREADYIGNORE), true)
            if LUIE.ChatAnnouncements.SV.Social.FriendIgnoreAlert then
                ZO_Alert(UI_ALERT_CATEGORY_ERROR, nil, (GetString(LUIE_SLASHCMDS_IGNORE_FAILED_ALREADYIGNORE)))
            end
