...
local CCT_ICONBORDER = "LuiExtended/media/cct/border.dds"
...
function CCT:OnOff()
	if CI.SV.cct.enabled and not (CI.SV.cct.enabledOnlyInCyro and LUIE.ResolvePVPZone()) then
		if not self.addonEnabled then
			...
			EVENT_MANAGER:RegisterForEvent(self.name, EVENT_PLAYER_ACTIVATED, self.Initialize)
		...
		if self.addonEnabled then
			...
			LUIE_CC_Tracker:SetHidden(true)
		end
	end
end
...
function LUIE_CC_Tracker_SavePosition()
	local coordX, coordY=LUIE_CC_Tracker:GetCenter()
	CI.SV.cct.offsetX=coordX-(GuiRoot:GetWidth()/2)
	CI.SV.cct.offsetY=coordY-(GuiRoot:GetHeight()/2)
	LUIE_CC_Tracker:ClearAnchors()
	LUIE_CC_Tracker:SetAnchor(CENTER, GuiRoot, CENTER, CI.SV.cct.offsetX, CI.SV.cct.offsetY)
end

function LUIE_CC_Tracker_OnUpdate(control)
	if CCT.Timer==0 or not CCT.Timer then return end

	local timeLeft = math.ceil(CCT.Timer - GetFrameTimeSeconds())
	if timeLeft>0 then
		LUIE_CC_Tracker_Timer_Label:SetText(timeLeft)
	end
end

function CCT:OnProc(ccDuration, interval)
	self:OnAnimation(LUIE_CC_Tracker, "proc")
	...
	if remaining>0 then
		LUIE_CC_Tracker_IconFrame_GlobalCooldown:ResetCooldown()
		if CI.SV.cct.showGCD and LUIE.ResolvePVPZone() then
			LUIE_CC_Tracker_IconFrame_GlobalCooldown:SetHidden(false)
			LUIE_CC_Tracker_IconFrame_GlobalCooldown:StartCooldown(remaining, remaining, CD_TYPE_RADIAL, CD_TIME_TYPE_TIME_UNTIL, false)
			zo_callLater(function() LUIE_CC_Tracker_IconFrame_GlobalCooldown:SetHidden(true) end, remaining)
		end
	end
	LUIE_CC_Tracker_IconFrame_Cooldown:ResetCooldown()
	LUIE_CC_Tracker_IconFrame_Cooldown:StartCooldown(interval, ccDuration, CD_TYPE_RADIAL, CD_TIME_TYPE_TIME_REMAINING, false)
...
function CCT:OnAnimation(control, animationType, param)
	...
	elseif param then LUIE_CC_Tracker:SetHidden(not CI.SV.cct.unlocked)
	end
end
...
function CCT:OnCombat(eventCode, result, isError, abilityName, abilityGraphic, abilityActionSlotType, sourceName, sourceType, targetName, targetType, hitValue, powerType, damageType, combat_log, sourceUnitId, targetUnitId, abilityId)
	...
	if not StringEnd(LUIE.PlayerNameRaw,'^Mx') and not StringEnd(LUIE.PlayerNameRaw,'^Fx') then malformedName = true end
	...
	if result==ACTION_RESULT_EFFECT_GAINED_DURATION and ((not malformedName and sourceName==LUIE.PlayerNameRaw) or (malformedName and (sourceName==LUIE.PlayerNameRaw..'^Mx' or sourceName==LUIE.PlayerNameRaw..'^Fx'))) and (abilityName=="Break Free" or abilityName==GetAbilityName(CCT_BREAK_FREE_ID) or abilityId==CCT_BREAK_FREE_ID) then
		if CI.SV.cct.showOptions=="text" then self:StopDraw(true)	else self:StopDrawBreakFree() end return
	end
	...
	if CI.SV.cct.showAoe and (self:AoePriority(abilityName, result) or (CC.SpecialCC[abilityId] and result == ACTION_RESULT_EFFECT_GAINED)) then
        ...
        if not CC.aoeTypesId[abilityId] then return end
        if CC.SpecialCC[abilityId] and result ~= ACTION_RESULT_EFFECT_GAINED then return end
        ...
        if CC.aoeTypesId[abilityId] <= 199 then
            if not CI.SV.cct.showAoeT1 then return end
        ...
        if CC.aoeTypesId[abilityId] >= 200 and CC.aoeTypesId[abilityId] <= 499 then
            if not CI.SV.cct.showAoeT2 then return end
        ...
        if CC.aoeTypesId[abilityId] >= 500 and CC.aoeTypesId[abilityId] <= 599 then
            if not CI.SV.cct.showAoeT3 then return end
        ...
        if CC.aoeTypesId[abilityId] >= 600 and CC.aoeTypesId[abilityId] <= 699 then
            if not CI.SV.cct.showAoeT4 then return end
        ...
        if CC.aoeTypesId[abilityId] >= 700 and CC.aoeTypesId[abilityId] <= 799 then
            if not CI.SV.cct.showAoeT5 then return end
        ...
        if CC.aoeTypesId[abilityId] >= 800 then
            if not CI.SV.cct.showAoeT6 then return end
	...
	if abilityName=="Hiding Spot" then return end -- TODO: Put ID here instead or add to blacklist instead
...
----AOE----
	...
	if CI.SV.cct.showOptions=="text" then stagger=true end
...
function CCT:OnDraw(abilityId, abilityIcon, ccDuration, result, abilityName, interval)
	...
	if result==ACTION_RESULT_STAGGERED then self:OnAnimation(LUIE_CC_Tracker, "stagger")	return end
	...
	if result==ACTION_RESULT_SILENCED or result==ACTION_RESULT_AREA_EFFECT then
		if CI.SV.cct.showOptions=="text" then
			self:OnAnimation(LUIE_CC_Tracker_TextFrame, "silence")
		else
			self:OnAnimation(LUIE_CC_Tracker_IconFrame, "silence")
		end
	elseif result==ACTION_RESULT_IMMUNE or result==ACTION_RESULT_DODGED or result==ACTION_RESULT_BLOCKED or result == ACTION_RESULT_BLOCKED_DAMAGE then
		self:OnAnimation(LUIE_CC_Tracker, "immune")
		if wasDefault then LUIE_CC_Tracker_IconFrame_Icon:SetTextureCoords(0.2,0.8,0.2,0.8) end
	else
...
function CCT:IconHidden(hidden)
	if CI.SV.cct.showOptions=="text" then
		LUIE_CC_Tracker_IconFrame:SetHidden(true)
	else
		LUIE_CC_Tracker_IconFrame:SetHidden(hidden)
	end
end

function CCT:TimerHidden(hidden)
	if CI.SV.cct.showOptions=="text" then
		LUIE_CC_Tracker_Timer:SetHidden(true)
	else
		LUIE_CC_Tracker_Timer:SetHidden(hidden)
	end
end

function CCT:TextHidden(hidden)
	if CI.SV.cct.showOptions=="icon" then
		LUIE_CC_Tracker_TextFrame:SetHidden(true)
	else
		LUIE_CC_Tracker_TextFrame:SetHidden(hidden)
	end
end

function CCT:BreakFreeHidden(hidden)
	if CI.SV.cct.showOptions=="text" then
		LUIE_CC_Tracker_BreakFreeFrame:SetHidden(true)
	else
		LUIE_CC_Tracker_BreakFreeFrame:SetHidden(hidden)
	end
end

function CCT:SetupInfo(ccText, ccColor, abilityIcon, wasDefault)
	LUIE_CC_Tracker_TextFrame_Label:SetFont(CCT_ICON_FONT)
	LUIE_CC_Tracker_TextFrame_Label:SetText(ccText)
	LUIE_CC_Tracker_TextFrame_Label:SetColor(unpack(ccColor))
	LUIE_CC_Tracker_IconFrame_Icon:SetTexture(abilityIcon)

	if wasDefault then
		LUIE_CC_Tracker_IconFrame_Icon:SetColor(unpack(ccColor))
	else
		LUIE_CC_Tracker_IconFrame_Icon:SetColor(1,1,1,1)
	end

	LUIE_CC_Tracker_IconFrame_IconBG:SetColor(unpack(ccColor))

	LUIE_CC_Tracker_IconFrame_IconBorder:SetColor(unpack(ccColor))
	LUIE_CC_Tracker_IconFrame_IconBorderHighlight:SetColor(unpack(ccColor))

	LUIE_CC_Tracker_Timer_Label:SetColor(unpack(ccColor))
end

function CCT:SetupDisplay(displayType)
	if displayType=="silence" then
		LUIE_CC_Tracker_IconFrame_Cooldown:SetHidden(true)
		LUIE_CC_Tracker_IconFrame_GlobalCooldown:SetHidden(true)
		LUIE_CC_Tracker_IconFrame_IconBorderHighlight:SetHidden(false)

		LUIE_CC_Tracker_IconFrame_Icon:SetTextureCoords(0,1,0,1)
		...
		LUIE_CC_Tracker:SetHidden(false)

	elseif displayType=="immune" then
		LUIE_CC_Tracker_IconFrame_Cooldown:SetHidden(true)
		LUIE_CC_Tracker_IconFrame_GlobalCooldown:SetHidden(true)
		LUIE_CC_Tracker_IconFrame_IconBorderHighlight:SetHidden(true)

		LUIE_CC_Tracker_IconFrame_Icon:SetTextureCoords(0,1,0,1)
		LUIE_CC_Tracker_IconFrame_IconBG:SetColor(0,0,0)
		self:IconHidden(false)
	...
	elseif displayType=="stagger" then
		LUIE_CC_Tracker_TextFrame_Label:SetText(CCT.controlText[ACTION_RESULT_STAGGERED])
		LUIE_CC_Tracker_TextFrame_Label:SetColor(unpack(CI.SV.cct.colors[ACTION_RESULT_STAGGERED]))
		LUIE_CC_Tracker_TextFrame_Label:SetFont(CCT_STAGGER_FONT)
		self:TextHidden(false)
	...
	elseif displayType=="timer" then
		LUIE_CC_Tracker_IconFrame_Cooldown:SetHidden(false)
...
function CCT:StopDraw(isTextOnly)
	...
	self:VarReset()
	if isTextOnly then self:OnAnimation(LUIE_CC_Tracker, "endstagger", true)
	else
		self:OnAnimation(LUIE_CC_Tracker, "end", true)
	end
end
...
function CCT:StopDrawBreakFree()
	...
	if currentCC~=0 and currentCC~=4 and currentCC~=6 then
		...
		LUIE_CC_Tracker_BreakFreeFrame_Left_IconBG:SetColor(unpack(currentColor))
		LUIE_CC_Tracker_BreakFreeFrame_Right_IconBG:SetColor(unpack(currentColor))
		LUIE_CC_Tracker_BreakFreeFrame_Left_IconBorder:SetColor(unpack(currentColor))
		LUIE_CC_Tracker_BreakFreeFrame_Left_IconBorderHighlight:SetColor(unpack(currentColor))
		LUIE_CC_Tracker_BreakFreeFrame_Right_IconBorder:SetColor(unpack(currentColor))
		LUIE_CC_Tracker_BreakFreeFrame_Right_IconBorderHighlight:SetColor(unpack(currentColor))
	end
	...
	if breakFreeIcon == CCT_ZOS_DEFAULT_ICON then
		...
		LUIE_CC_Tracker_BreakFreeFrame_Left_Icon:SetColor(unpack(CI.SV.cct.colors[self.controlTypes[currentCC]]))
		LUIE_CC_Tracker_BreakFreeFrame_Right_Icon:SetColor(unpack(CI.SV.cct.colors[self.controlTypes[currentCC]]))
	else
		LUIE_CC_Tracker_BreakFreeFrame_Left_Icon:SetColor(1,1,1,1)
		LUIE_CC_Tracker_BreakFreeFrame_Right_Icon:SetColor(1,1,1,1)
	end
	LUIE_CC_Tracker_BreakFreeFrame_Left_Icon:SetTexture(breakFreeIcon)
	LUIE_CC_Tracker_BreakFreeFrame_Left_Icon:SetTextureCoords(0,0.5,0,1)
	LUIE_CC_Tracker_BreakFreeFrame_Right_Icon:SetTexture(breakFreeIcon)
	LUIE_CC_Tracker_BreakFreeFrame_Right_Icon:SetTextureCoords(0.5,1,0,1)
...
function CCT:BreakFreeAnimation()
	...
	local leftSide, rightSide=LUIE_CC_Tracker_BreakFreeFrame_Left, LUIE_CC_Tracker_BreakFreeFrame_Right

	LUIE_CC_Tracker:SetScale(CI.SV.cct.controlScale)
	leftSide:ClearAnchors()
	leftSide:SetAnchor(RIGHT, LUIE_CC_Tracker_BreakFreeFrame_Middle, LEFT, 1-20, 0)
	rightSide:ClearAnchors()
	rightSide:SetAnchor(LEFT, LUIE_CC_Tracker_BreakFreeFrame_Middle, RIGHT, -1+20, 0)
	...
	timeline:SetHandler('OnStop', function()
		leftSide:ClearAnchors()
		leftSide:SetAnchor(LEFT, LUIE_CC_Tracker_BreakFreeFrame, LEFT, 0, 0)
		leftSide:SetScale(1)
		rightSide:ClearAnchors()
		rightSide:SetAnchor(RIGHT, LUIE_CC_Tracker_BreakFreeFrame, RIGHT, 0, 0)
...
function CCT:StartAnimation(control, animType, test)
	...
	elseif animType=="silence" then
		if LUIE_CC_Tracker:GetAlpha()<1 then
			self:InsertAnimationType(timeline, ANIMATION_ALPHA, LUIE_CC_Tracker, 100,   0, ZO_EaseInQuadratic,    0,   1)
...
function CCT:InitControls()
	...
	if CI.SV.cct.unlocked then
		LUIE_CC_Tracker_TextFrame_Label:SetText("Unlocked")
	else
		LUIE_CC_Tracker_TextFrame_Label:SetText("")
	end
	self:TextHidden(false)

	LUIE_CC_Tracker_IconFrame_IconBorder:SetTexture(CCT_ICONBORDER)
	LUIE_CC_Tracker_IconFrame_IconBorderHighlight:SetTexture(CCT_ICONBORDER)
	LUIE_CC_Tracker_IconFrame_IconBorder:SetHidden(false)
	LUIE_CC_Tracker_IconFrame_IconBorderHighlight:SetHidden(false)
	...
	LUIE_CC_Tracker_IconFrame_GlobalCooldown:ResetCooldown()
	LUIE_CC_Tracker_IconFrame_GlobalCooldown:SetHidden(true)
	LUIE_CC_Tracker_IconFrame_Icon:SetTexture(CCT_DEFAULT_IMMUNE_ICON)
	LUIE_CC_Tracker_IconFrame_Icon:SetTextureCoords(0.2,0.8,0.2,0.8)
	LUIE_CC_Tracker_IconFrame_IconBG:SetColor(1,1,1)
	LUIE_CC_Tracker_IconFrame_Icon:SetColor(1,1,1)
	self:IconHidden(false)

	LUIE_CC_Tracker_IconFrame_IconBorder:SetColor(1,1,1)
	LUIE_CC_Tracker_IconFrame_IconBorderHighlight:SetColor(1,1,1)
	LUIE_CC_Tracker_TextFrame_Label:SetColor(1,1,1)

	LUIE_CC_Tracker:SetMouseEnabled(CI.SV.cct.unlocked)
	LUIE_CC_Tracker:SetMovable(CI.SV.cct.unlocked)
	LUIE_CC_Tracker:SetAlpha(1)


	LUIE_CC_Tracker_BreakFreeFrame_Left_IconBorder:SetTexture(CCT_ICONBORDER)
	LUIE_CC_Tracker_BreakFreeFrame_Left_IconBorderHighlight:SetTexture(CCT_ICONBORDER)
	LUIE_CC_Tracker_BreakFreeFrame_Left_IconBorder:SetTextureCoords(0,0.5,0,1)
	LUIE_CC_Tracker_BreakFreeFrame_Left_IconBorderHighlight:SetTextureCoords(0,0.5,0,1)

	LUIE_CC_Tracker_BreakFreeFrame_Right_IconBorder:SetTexture(CCT_ICONBORDER)
	LUIE_CC_Tracker_BreakFreeFrame_Right_IconBorderHighlight:SetTexture(CCT_ICONBORDER)
	LUIE_CC_Tracker_BreakFreeFrame_Right_IconBorder:SetTextureCoords(0.5,1,0,1)
	LUIE_CC_Tracker_BreakFreeFrame_Right_IconBorderHighlight:SetTextureCoords(0.5,1,0,1)

	LUIE_CC_Tracker_BreakFreeFrame_Left_Icon:SetTexture(CCT_DEFAULT_DISORIENT_ICON)
	LUIE_CC_Tracker_BreakFreeFrame_Left_Icon:SetTextureCoords(0,0.5,0,1)
	LUIE_CC_Tracker_BreakFreeFrame_Right_Icon:SetTexture(CCT_DEFAULT_DISORIENT_ICON)
	LUIE_CC_Tracker_BreakFreeFrame_Right_Icon:SetTextureCoords(0.5,1,0,1)

	self:BreakFreeHidden(true)

	self:TimerHidden(not CI.SV.cct.unlocked)
	LUIE_CC_Tracker_Timer_Label:SetText("69")
	LUIE_CC_Tracker_Timer_Label:SetColor(1,1,1,1)
	LUIE_CC_Tracker:SetHidden(not CI.SV.cct.unlocked)
end
