...
function CombatTextCombatEventListener:OnCombatIn(...)
    ...
    local isDamage, isDamageCritical, isDot, isDotCritical =
        CombatTextConstants.isDamage[resultType],
        CombatTextConstants.isDamageCritical[resultType],
        CombatTextConstants.isDot[resultType],
        CombatTextConstants.isDotCritical[resultType]
    --Healing
    local isHealing, isHealingCritical, isHot, isHotCritical =
        CombatTextConstants.isHealing[resultType],
        CombatTextConstants.isHealingCritical[resultType],
        CombatTextConstants.isHot[resultType],
        CombatTextConstants.isHotCritical[resultType]
    --Energize & Drain
    ...
    local isMiss, isImmune, isParried, isReflected, isDamageShield, isDodged, isBlocked, isInterrupted =
        CombatTextConstants.isMiss[resultType],
        CombatTextConstants.isImmune[resultType],
        CombatTextConstants.isParried[resultType],
        CombatTextConstants.isReflected[resultType],
        CombatTextConstants.isDamageShield[resultType],
        CombatTextConstants.isDodged[resultType],
        CombatTextConstants.isBlocked[resultType],
        CombatTextConstants.isInterrupted[resultType]
    --Crowd Control
    local isDisoriented, isFeared, isOffBalanced, isSilenced, isStunned, isCharmed =
        CombatTextConstants.isDisoriented[resultType],
        CombatTextConstants.isFeared[resultType],
        CombatTextConstants.isOffBalanced[resultType],
        CombatTextConstants.isSilenced[resultType],
        CombatTextConstants.isStunned[resultType],
        CombatTextConstants.isCharmed[resultType]
    --Overflow
    local overkill, overheal =
        (Settings.common.overkill and overflow > 0 and (isDamage or isDamageCritical or isDot or isDotCritical)),
        (Settings.common.overheal and overflow > 0 and (isHealing or isHealingCritical or isHot or isHotCritical))
    ---------------------------------------------------------------------------------------------------------------------------------------
    --//COMBAT TRIGGERS//--
    ---------------------------------------------------------------------------------------------------------------------------------------
    if
        ...
        or (
            isDrain
            and togglesInOut.showDrain
            and (powerType == COMBAT_MECHANIC_FLAGS_MAGICKA or powerType == COMBAT_MECHANIC_FLAGS_STAMINA)
        )
    then
    ...
    if isWarned.combat then --Only show CC/Debuff events when in combat
        ...
        if isDisoriented and togglesInOut.showDisoriented then
            if isWarned.disoriented then
                ...
                self:TriggerEvent(
                    CombatTextConstants.eventType.CROWDCONTROL,
                    CombatTextConstants.crowdControlType.DISORIENTED,
                    combatType
                )
        ...
        if isFeared and togglesInOut.showFeared then
            if isWarned.feared then
                ...
                self:TriggerEvent(
                    CombatTextConstants.eventType.CROWDCONTROL,
                    CombatTextConstants.crowdControlType.FEARED,
                    combatType
                )
        ...
        if isOffBalanced and togglesInOut.showOffBalanced then
            if isWarned.offBalanced then
                ...
                self:TriggerEvent(
                    CombatTextConstants.eventType.CROWDCONTROL,
                    CombatTextConstants.crowdControlType.OFFBALANCED,
                    combatType
                )
        ...
        if isSilenced and togglesInOut.showSilenced then
            if isWarned.silenced then
                ...
                self:TriggerEvent(
                    CombatTextConstants.eventType.CROWDCONTROL,
                    CombatTextConstants.crowdControlType.SILENCED,
                    combatType
                )
        ...
        if isStunned and togglesInOut.showStunned then
            if isWarned.stunned then
                ...
                self:TriggerEvent(
                    CombatTextConstants.eventType.CROWDCONTROL,
                    CombatTextConstants.crowdControlType.STUNNED,
                    combatType
                )
        ...
        if isCharmed and togglesInOut.showCharmed then
            if isWarned.charmed then
                ...
                self:TriggerEvent(
                    CombatTextConstants.eventType.CROWDCONTROL,
                    CombatTextConstants.crowdControlType.CHARMED,
                    combatType
                )
