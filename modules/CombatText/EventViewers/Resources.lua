...
function CombatTextResourceEventViewer:OnEvent(resourceType, value)
    ...
    if resourceType == resourceTypes.LOW_HEALTH then
        ...
        text = self:FormatString(
            Settings.formats.resourceHealth,
            { value = value, text = GetString(SI_LUIE_LAM_CT_SHARED_LOW_HEALTH) }
        )
    --Low Magicka
    elseif resourceType == resourceTypes.LOW_MAGICKA then
        ...
        size = Settings.fontSizes.resource
        text = self:FormatString(
            Settings.formats.resourceMagicka,
            { value = value, text = GetString(SI_LUIE_LAM_CT_SHARED_LOW_MAGICKA) }
        )
    --Low Stamina
    elseif resourceType == resourceTypes.LOW_STAMINA then
        ...
        size = Settings.fontSizes.resource
        text = self:FormatString(
            Settings.formats.resourceStamina,
            { value = value, text = GetString(SI_LUIE_LAM_CT_SHARED_LOW_STAMINA) }
        )
    --Ultimate Ready
    elseif resourceType == resourceTypes.ULTIMATE then
        ...
        text = self:FormatString(
            Settings.formats.ultimateReady,
            { text = GetString(SI_LUIE_LAM_CT_SHARED_ULTIMATE_READY) }
        )
    -- Potion Ready
    ...
    if
        resourceType == resourceTypes.LOW_HEALTH
        or resourceType == resourceTypes.LOW_MAGICKA
        or resourceType == resourceTypes.LOW_STAMINA
    then
