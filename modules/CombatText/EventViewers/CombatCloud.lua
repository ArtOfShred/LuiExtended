...
function CombatTextCombatCloudEventViewer:OnEvent(
    combatType,
    powerType,
    value,
    abilityName,
    ...
    if
        (isDamageCritical or isHealingCritical or isDotCritical or isHotCritical)
        and not Settings.toggles.throttleCriticals
    then
        self:View(
            combatType,
            powerType,
        ...
        local eventKey = string.format(
            ...
            sourceName,
            tostring(isDamage),
            tostring(isDamageCritical),
            tostring(isHealing),
            tostring(isHealingCritical),
            tostring(isEnergize),
            tostring(isDrain),
            tostring(isDot),
            tostring(isDotCritical),
            tostring(isHot),
            tostring(isHotCritical),
            tostring(isMiss),
            tostring(isImmune),
            tostring(isParried),
            tostring(isReflected),
            tostring(isDamageShield),
            tostring(isDodged),
            tostring(isBlocked),
            tostring(isInterrupted)
        )
...
function CombatTextCombatCloudEventViewer:ViewFromEventBuffer(
    combatType,
    powerType,
    eventKey,
    abilityName,
...
function CombatTextCombatCloudEventViewer:View(
    combatType,
    powerType,
    value,
    abilityName,
    ...
    if isDamageCritical or isHealingCritical or isDotCritical or isHotCritical then
        offsetX, offsetY = math.random(-radiusW * 0.5, radiusW * 0.5), math.random(-radiusH * 0.5, radiusH * 0.5)
    elseif isDot or isHot then -- http://www.mathopenref.com/coordgeneralellipse.html
        offsetX = math.random(-radiusW * 0.95, radiusW * 0.95) -- Make radiusW a bit smaller to avoid horizontal animations
    ...
    elseif isDamage or isHealing or isEnergize or isDrain or isDamageShield or isBlocked then
        offsetX, offsetY = math.random(-radiusW, radiusW), math.random(-radiusH * 0.5, radiusH)
    end
    ...
    local textFormat, fontSize, textColor = self:GetTextAtributes(
        powerType,
        damageType,
        isDamage,
        isDamageCritical,
    ...
    if
        (combatType == CombatTextConstants.combatType.INCOMING)
        and Settings.toggles.incomingDamageOverride
        and (isDamage or isDamageCritical)
    then
    ...
    self:PrepareLabel(
        ...
        self:FormatString(
            textFormat,
            { text = abilityName, value = value, powerType = powerType, damageType = damageType }
        )
    )
