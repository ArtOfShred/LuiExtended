...
function SlashCommands.SlashFriend(option)
    if option == "" or option == nil then
        printToChat(GetString(LUIE_SLASHCMDS_FRIEND_FAILED_NONAME), true)
        if LUIE.ChatAnnouncements.SV.Social.FriendIgnoreAlert then
            ZO_Alert(UI_ALERT_CATEGORY_ERROR, nil, (GetString(LUIE_SLASHCMDS_FRIEND_FAILED_NONAME)))
        end
...
function SlashCommands.SlashIgnore(option)
    if option == "" or option == nil then
        printToChat(GetString(LUIE_SLASHCMDS_IGNORE_FAILED_NONAME), true)
        if LUIE.ChatAnnouncements.SV.Social.FriendIgnoreAlert then
            ZO_Alert(UI_ALERT_CATEGORY_ERROR, nil, (GetString(LUIE_SLASHCMDS_IGNORE_FAILED_NONAME)))
        end
        PlaySound(SOUNDS.GENERAL_ALERT_ERROR)
        return
    end
...
function SlashCommands.SlashRemoveFriend(option)
    if option == "" or option == nil then
        printToChat(GetString(LUIE_SLASHCMDS_FRIEND_REMOVE_FAILED_NONAME), true)
        if LUIE.ChatAnnouncements.SV.Social.FriendIgnoreAlert then
            ZO_Alert(UI_ALERT_CATEGORY_ERROR, nil, (GetString(LUIE_SLASHCMDS_FRIEND_REMOVE_FAILED_NONAME)))
        end
        PlaySound(SOUNDS.GENERAL_ALERT_ERROR)
        return
    end
...
function SlashCommands.SlashRemoveIgnore(option)
    if option == "" or option == nil then
        printToChat(GetString(LUIE_SLASHCMDS_IGNORE_FAILED_NONAME_REMOVE), true)
        if LUIE.ChatAnnouncements.SV.Social.FriendIgnoreAlert then
            ZO_Alert(UI_ALERT_CATEGORY_ERROR, nil, (GetString(LUIE_SLASHCMDS_IGNORE_FAILED_NONAME_REMOVE)))
        end
        PlaySound(SOUNDS.GENERAL_ALERT_ERROR)
        return
    end
